...
# Iterate over all directories and add import to $1 index.ts
for file in *
do
    if [ "$2" == "types" ]
    then
        fileName="${file/.[a-ZA-Z]*/}";

        if [ "$fileName" != "index" ]
        then
            echo "export type * from \"./$fileName\";" >> "index.ts"
        fi
    else
        if [ -d "$file" ]
        then
            echo "export type * from \"./$file\";" >> "index.ts"
            echo "export { default as $file } from \"./$file\";" >> "index.ts"
        fi
    fi
done