...
describe("<Sections />", () => {
  ...
  it("has the organisations permissions link", async () => {
    ...
    await waitFor(() => {
      expect(permissions[0]).toHaveAttribute(
        "href",
        `/en${ROUTES.permissionsOrganisationDataCustodian.path}/1`
      );
    });
  });

  it("has the user permissions link", async () => {
    const permissions = await setupActionMenuTest(
    ...
    await waitFor(() => {
      expect(permissions[0]).toHaveAttribute(
        "href",
        `/en${ROUTES.permissionsUserDataCustodian.path}/1`
      );
    });
  });

  it("deletes the approval of a user", async () => {
  ...
  it("shows a user's details", async () => {
